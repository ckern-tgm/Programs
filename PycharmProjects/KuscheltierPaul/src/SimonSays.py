    ...
    def getSimonSays(self):
        ...
        while self.notfall == False and self.abbr == False:
            ...
            if ausgabe['zahl'] <= 4 and input > 0:
                ...
                engine.say("Wenn Sie noch einmal spielen wollen, drücken Sie die rechte Hand. Sonst drücken Sie die linke Hand")
                engine.runAndWait()
                if self.rHand == True:
                    engine.say("Sie haben noch einmal spielen gewählt")
                    engine.runAndWait()
                    break
                elif self.lHand == True:
                    engine.say("Sie haben das Spiel verlassen")
                    engine.runAndWait()
                    return
