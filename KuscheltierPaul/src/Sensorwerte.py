    ...
    def update(self):
        with connect(process_setup=False) as hedgehog:
            while True:
                ...
                self.rFuss = hedgehog.get_digital(3)
                hedgehog.set_input_state(3,True)
                self.lFuss = hedgehog.get_digital(4)
                hedgehog.set_input_state(4,True)
                hedgehog.set_input_state(5,True)
                self.lOhr = hedgehog.get_digital(7)
                self.pulsAnalog = hedgehog.get_analog(7)
                self.abbr = hedgehog.get_digital(6)
                hedgehog.set_input_state(6,True)
                self.notfall = hedgehog.get_digital(0)
                hedgehog.set_input_state(0,True)
                self.akku = hedgehog.get_analog(128)
                time.sleep(0.1) 
